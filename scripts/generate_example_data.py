...
def random_death_date(era: str) -> datetime.date:
    """Generate random death date within an era."""
    ranges = {
...
def random_buddhist_name(is_female: bool) -> str:
    """Generate a random Buddhist posthumous name."""
...
def format_era_date(d: datetime.date) -> str:
    """Format date in Japanese era style."""
    from memorial_app.core.era_converter import gregorian_to_era, format_era_year
...
def generate_records(n: int) -> list[dict]:
    """Generate n random person records."""
    era_weights = {
        "meiji": 5,
        "taisho": 8,
        "showa_early": 20,
    ...
    for _ in range(n):
        ...
        family_given = random.choice(GIVEN_NAMES_M)
        family = f"{family_surname}　{family_given}"
...
def build_sheet1(records: list[dict]) -> pd.DataFrame:
    """Sheet 1: 寺院記録 - Formal style with era dates."""
    rows = []
    for r in records:
        rows.append({
            "氏名": r["name"],
...
def build_sheet2(records: list[dict]) -> pd.DataFrame:
    """Sheet 2: 管理台帳 - Informal style with Western dates."""
    rows = []
    for r in records:
        rows.append({
            "お名前": r["name"],
...
def build_sheet3(records: list[dict]) -> pd.DataFrame:
    """Sheet 3: DATA - Mixed style with some era, some Western."""
    rows = []
...
def main():
    ...
    output_path = output_dir / "example_dataset.xlsx"

    random.seed(42)  # Reproducible

    records = generate_records(100)

    # Split records across 3 sheets: 40, 35, 25
    ...
    print(f"  Sheet '寺院記録': {len(sheet1_recs)} records (era format dates)")
    print(f"  Sheet '管理台帳': {len(sheet2_recs)} records (Western format dates)")
    print(f"  Sheet 'DATA': {len(sheet3_recs)} records (mixed format dates)")
